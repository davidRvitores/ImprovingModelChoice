...
# save(SigmaMatrixClas, XmatrixClas,labelsMatrixClas,muCPC,SigmaCPC,Xcpc,labelsCPC,muPROP,SigmaPROP,Xprop,labelsPROP, 
# file= "simulationData.RData")
load("/home/david/DATOS/simulationData.RData")
...
solCPC <- GCPCclas(XmatrixClas,labelsMatrixClas,M=4,csh=100,cvol=100,niter=100,tol=10^(-5),nstart=5,graph=F) # Common principal components solution 
...
for(i in 1:M){
  ind <- which(grupo==i)
  L <- length(ind)
  plot(ellipse(Sn[[ind[1]]],centre=c(0,0)),col=i+1,type="l",xlim=c(-4,4),ylim=c(-4,4),
       xlab="",ylab="",lwd=0.9)
  if(L>1){
    for(l in 1:L){
      lines(ellipse(Sn[[ind[l]]],centre=c(0,0),npoints=500),col=i,lwd=0.9)
    }
  }
  for(j in 1:2){
...
solPROP <- GPROPclas(XmatrixClas,labelsMatrixClas,M=4,csh=100,cvol=100,niter=100,tol=10^(-5),nstart=5,graph=F)  # Proportionality solution
...
for(i in 1:M){
  ...
  Sigma[[i]] <- Sigma[[i]]/det(Sigma[[i]])^(1/d)
}
...
for(i in 1:M){
  ind <- which(grupo==i)
  L <- length(ind)
  plot(ellipse(Sn[[ind[1]]],centre=c(0,0)),col=i+4,type="l",xlim=c(-4,4),ylim=c(-4,4),
       xlab="",ylab="",lwd=0.9)
  if(L>1){
    for(l in 1:L){
      lines(ellipse(Sn[[ind[l]]],centre=c(0,0),npoints=500),col=i+4,lwd=0.9)
    }
  }
  for(j in 1:2){
...
for(l in 1:m){
  ...
  solCPC <- GCPC(Xcpc,k=6,M=2,csh=100,cvol=100,niter=200,tol=10^(-8),nstart=10,graph=F)
  sink()
  bicCPC <- solCPC$BIC
  sink("NUL")
  mod <- Mclust(X,BIC=bic,G=6)
  sink()
  bicMCLUST <-  mod$bic
  ...
  # Discriminant analysis
  sink("NUL")
  solCPC <- GCPCclas(Xcpc,labelsCPC,M=2,csh=100,cvol=100,niter=100,tol=10^(-5),nstart=10,graph=F)
  sink()
  bicCPC <- solCPC$BIC
  sink("NUL")
